...
func Resources(t testing.TB) []dtypes.Resource {
	...
	for i := 0; i < count; i++ {
		coin := sdk.NewCoin(CoinDenom, sdk.NewInt(int64(rand.Uint16())))
		if coin.Amount.Int64() > int64(10000) {
			limited := coin.Amount.Int64() % int64(1000)
			coin = sdk.NewCoin(CoinDenom, sdk.NewInt(limited))
		}
		res := dtypes.Resource{
			Unit: types.Unit{
				CPU:     100,
